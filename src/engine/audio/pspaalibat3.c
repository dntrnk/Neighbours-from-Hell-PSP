...
int LoadAt3(const char* filename,int channel)
{
	if ((channel<0)||(channel>1))
	{
		return PSPAALIB_ERROR_AT3_INVALID_CHANNEL;
	...
	streamsAt3[channel].paused=1;
	streamsAt3[channel].autoloop=0;
	streamsAt3[channel].initialized=1;
...
int UnloadAt3(int channel)
{
	...
	streamsAt3[channel].initialized=0;
